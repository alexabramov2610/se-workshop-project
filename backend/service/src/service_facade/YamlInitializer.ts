...
// }
//
// const systemInit = async (username: string, password: string): void => {
//     adminToken = getSession();
//     const initReq: Req.InitReq = {  body: { firstAdminName: username, firstAdminPassword: password } , token: adminToken};
//     const res: BoolResponse = await ServiceFacade.systemInit(initReq).data.result
//     if (!res)
...
//     const res = await ServiceFacade.registerUser(regReq).data.result
//     if (!res)
